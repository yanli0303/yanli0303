...
export const getShieldImageFileName = (profile: Profile, style: string, format: ShieldFormat) => {
  ...
  for (const style of styles) {
    for (const profile of config.profiles) {
      ...
      for (const format of Object.values(ShieldFormat)) {
        ...
        const rightText = format === ShieldFormat.iconOnly ? "" : profile.username;
        const rightBackgroundColor =
          format === ShieldFormat.labelled ? profile.rightBackgroundColor : profile.leftBackgroundColor;
        const fileName = getShieldImageFileName(profile, style, format);
        const saveAs = join(dir, fileName);
